...
and architcture are done and we have an estimate series of tasks, What's the
process?
...
lets add a click handler and make an XHR.  The XHR is returning a 404?  Ok,
that means the next step is to add a route to the http server.  Lets sketch out
a skeleton that returns a '400' aka Bad Request, since we don't know how to
handle it yet.  
...
login button and the XHR has returned to the front-end.

We can now strt to inspect the request, pull out the email address and password
and then call out to the database to see if it matches a valid user account.
...
At every step of this process we're interacting with the platform just like the
user would.  By the time we've written the code we've run it.  We've run it in
the stack where it's supposed to run.  Nothing was mocked out.
